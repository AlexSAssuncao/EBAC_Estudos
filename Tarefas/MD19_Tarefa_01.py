...
@st.cache_data
def to_excel(df):
    output = BytesIO()  # Criar buffer de memória
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        df.to_excel(writer, index=False, sheet_name='Dados')  # Salvar sem índice
    processed_data = output.getvalue()  # Obter bytes do arquivo
...
def main():
    ...
    if (data_file_1 is not None):
        ...
        if graph_type == 'Barras':
            ...
            ax[0].set_title('Dados brutos')
            ax[0].legend(loc="upper left", bbox_to_anchor=(0, 0))
            
            bank_target_perc.plot(kind='pie', autopct='%.2f', y='proportion', ax = ax[1])
            ax[1].set_title('Dados filtrados')
            ax[1].legend(loc="upper left", bbox_to_anchor=(0, 0))
